---
name: publish
description: 把创作者的原始输入（对话记录 / 个人笔记 / 博客草稿）转化为本站的超媒介 MDX 页面。当用户消息首行以 /publish 或 发布： 开头时必须使用本 skill。它是 docs/MEDIUM.md 创作宪法的可执行形态。
---

# /publish —— 剧本翻译成视频

用户负责写剧本（对话 / 笔记 / 草稿），你负责把它翻译成本站的超媒介形态。
