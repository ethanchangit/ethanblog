...
- `npm run studio`（或 `npm run dev`）在 http://localhost:4321/studio 打开本地写作台：创建文章/项目/系列、Markdown 编辑、站点预览、收入 `/blogs`、提交 `src/content`。该路由与 `/__studio/api` 只在 `astro dev` 注入，生产构建不打包。
