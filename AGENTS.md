...
Cloud agent 不使用后台存在 D1 里的授权。在 https://cursor.com/agents 的 MCP 菜单添加 HTTP 服务器 `https://api.heptabase.com/mcp` 并完成 OAuth（scopes：`offline_access space:read space:write`）。不要提交令牌，也不要把 `STUDIO_SECRET` 复制进 agent 环境。
