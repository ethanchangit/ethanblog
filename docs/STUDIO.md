...
6. 只在 blog 上设置 ✓ 和 ×。通过前明确确认正文与所有 page 都可公开；立即回写 `published`，空的 `Publish Date` 补当天日期（默认 Africa/Dar_es_Salaam），已有日期保留。此时仅为「已通过，待发布」，尚未上传 GitHub。拒绝则回写 `block`，保留日期和线上旧文章，不填写 comment。被引用的其他 blog 必须单独通过，不能借 mention 绕过拒绝。
...
引用资料有独立页面，桌面在右侧阅读，手机或无 JavaScript 时正常跳转；没有 `#blog` 的资料不进主文章列表，但正文进入全文搜索。草稿不进入公开站点或搜索。
