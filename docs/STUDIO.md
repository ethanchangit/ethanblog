...
3. 左侧分成 **New articles**（新卡片或首次公开）、**Edited articles**（GitHub 主版本里已公开文章的更新）与 **Deleted articles**（移出 `#blog` 或源卡片已删除）。blog 用有序清单，跟随的 page 用下方无序清单。首次关联可选择已有网页，不按标题猜测。没有待删除文章时不显示第三组。
...
已有对应卡片：从卡片列表拉取，选择原文章。没有对应卡片：在「首次关联旧文章」里点「创建 Heptabase 卡片」，核对后创建，并加入 `#blog`。这只建立对应关系，仍需拉取和审查才能提交。
