...
在 Heptabase 将关联文章移出 `#blog`，或直接删除源卡片，下次「拉取最新更新」会把它列入 **Deleted articles**。关于、Now、联系、隐私同样：卡片离开 `#blog` 或被删除就进入这份清单。只留在 `references` 标签里的卡片不是删除。这项检查不要求卡片仍为 Review，也包括已经通过审核、尚未上线的新文章。没有关联源卡片的旧文章不会因为拉取结果里找不到它而被删除。权限、网络或不完整列表不当作删除。不会删除 Heptabase 里的源卡片。
