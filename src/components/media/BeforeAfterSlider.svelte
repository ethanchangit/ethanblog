...
<figure class="media-frame not-prose">
  ...
  >
    ...
    <div class="pointer-events-none absolute inset-y-0" style="left: {pos}%">
      <div class="absolute inset-y-0 -ml-px w-px bg-ink-500"></div>
      <div
        class="absolute top-1/2 -ml-3 flex h-6 w-6 -translate-y-1/2 items-center justify-center text-[10px] text-ink-300"
      >
        ⇔
      </div>
    </div>

    <span class="absolute left-3 top-3 text-xs text-ink-400">{labels[0]}</span>
    <span class="absolute right-3 top-3 text-xs text-ink-400">{labels[1]}</span>
