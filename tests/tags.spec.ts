...
test.describe('Tags（内容集合过滤）', () => {
  ...
  test('文章页眉标签可点，在 /tags 就地筛出该标签文档', async ({ page }) => {
    ...
    const tag = page.locator('a[href="/tags?tag=PKM"]');
  ...
  test('旧 /tags/{tag} 地址重定向到查询参数', async ({ page }) => {
    await page.goto('/tags/PKM', { waitUntil: 'domcontentloaded' });
    await expect.poll(() => selectedTag(page.url())).toBe('PKM');
    await expect(page.getByRole('heading', { name: TITLE })).toBeVisible();
  });

  test('草稿专属标签不生成页面', async ({ page }) => {
    const response = await page.goto(`/tags/${encodeURIComponent('Realtalk')}`, { waitUntil: 'domcontentloaded' });
