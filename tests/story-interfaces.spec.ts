...
const FINAL = '/stories/how-this-site-works/';
const NOTE = '/stories/notes/web-as-medium/01-medium-engine-groundwork/';

test.describe('Story 论文化接口（T2）', () => {
  test('定稿页有摘要小标、署名行与引用块', async ({ page }) => {
    const response = await page.goto(FINAL);
    expect(response?.status()).toBe(200);

    const header = page.locator('article header');
    await expect(header.getByText('摘要', { exact: true })).toBeVisible();
    await expect(header.getByText(/文 \//)).toBeVisible();
  ...
  test('1440×900 下悬挂目录可见，点击锚点后 URL 带 #', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto(FINAL);

    const toc = page.locator('nav[aria-label="目录"]');
  ...
  test('notebook 页有研究线眉头与非定稿说明，无引用块，文末有 mailto', async ({ page }) => {
    const response = await page.goto(NOTE);
    expect(response?.status()).toBe(200);

    const masthead = page.locator('article header a[href*="/threads/web-as-medium"]');
    await expect(masthead).toBeVisible();
    await expect(masthead).toContainText('研究线：把网页当动态媒介');

    await expect(page.getByText('非定稿')).toBeVisible();
