...
test.describe('Theme（浅色/深色）', () => {
  ...
  test('页脚切换主题并持久化', async ({ page }) => {
    ...
    const toggle = footer.getByRole('button', { name: "Toggle light or dark mode" });
    const island = footer.locator('astro-island').filter({
      has: page.getByRole('button', { name: "Toggle light or dark mode" }),
    });
  ...
  test('页脚钉在视口底，滚动后仍可点', async ({ page }) => {
    ...
    await expect(footer).toBeInViewport();
    await expect(footer.getByRole('button', { name: "Toggle light or dark mode" })).toBeVisible();
  ...
  test('主题按钮在页脚、只有图标、没有「深色/浅色」文字', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: "Toggle light or dark mode" })).toHaveCount(1);
    await expect(page.locator('header').getByRole('button', { name: "Toggle light or dark mode" })).toHaveCount(0);
